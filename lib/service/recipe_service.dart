...
class RecipeService {
  ...
  Future<ApiResponseModel> queryRecipesByName(String query) async {
    try {
      final rawUri = url + "/Recipe/Query/$query";
  ...
  Future<ApiResponseModel> getPlannedMeals({
    int pageNumber = 1,
    int pageSize = 50,
  }) async {
    try {
      final rawUri = url + "/Recipe/MealPlanner";
      ...
      if (response.statusCode == 200) {
        ...
        return ApiResponseModel.success(
          response.statusCode,
          List<PlannedMealModel>.from(
            jsonBody.map((model) => PlannedMealModel.fromJson(model)),
          ),
        );
      } else {
        return ApiResponseModel.error(
  ...
  Future<ApiResponseModel> createNewMealPlanning(PlannedMealModel meal) async {
    try {
      final rawUri = url + "/Recipe/MealPlanner";
      ...
      var response = await http
          .post(
            uri,
            headers: header,
            body: jsonEncode(meal.toCreateJson()),
          )
          .timeout(Global.timeoutDuration);
