...
class AuthenticationState with ChangeNotifier {
  ...
  Future<int> signIn({String? email, String? password, String? encodedCredentials}) async {
    Map<String, dynamic> response;
    
    if(encodedCredentials != null){
      response = await AuthenticationService().signInWithEmailAndPassword(encodedCredentials: encodedCredentials);
    }
    else{
      response = await AuthenticationService().signInWithEmailAndPassword(email: email, password: password);
    }
